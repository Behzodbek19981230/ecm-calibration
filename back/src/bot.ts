...
function registerHandlers(b: TelegramBot): void {
	b.onText(/\/start (.+)/, async (msg, match) => {
		...
		b.sendMessage(
			chatId,
			`✅ Salom, ${name}!\n\nSiz ECM CALIBRATION tizimiga muvaffaqiyatli ulanyapsiz. Sertifikat va natijalar shu yerga yuboriladi.`,
		);
	});
	...
	b.onText(/\/start$/, (msg) => {
		b.sendMessage(
			msg.chat.id,
			'Assalomu alaykum! Bu ECM CALIBRATION bildirishnoma botidir.\n\nAriza berishda "Telegram orqali ulash" tugmasini bosing.',
		);
	});
}
