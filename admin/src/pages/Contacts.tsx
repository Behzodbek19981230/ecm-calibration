...
export default function Contacts() {
  ...
  async function markRead(id: number) {
    ...
    setContacts((prev) => prev.map((x) => x.id === id ? { ...x, isRead: true } : x));
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-1 justify-end">
                        <button onClick={() => { setSelected(contact); if (!contact.isRead) markRead(contact.id); }} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200 transition-all" title={t.common.view}>
                          <Eye size={14} />
                        </button>
                        {!contact.isRead && (
                          <button onClick={() => markRead(contact.id)} className="p-1.5 rounded-lg hover:bg-green-50 dark:hover:bg-green-950/50 text-gray-500 dark:text-slate-400 hover:text-green-600 transition-all" title={c.markRead}>
                            <CheckCheck size={14} />
                          </button>
                        )}
                        <button onClick={() => deleteContact(contact.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/50 text-gray-500 dark:text-slate-400 hover:text-red-500 transition-all" title={t.common.delete}>
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
